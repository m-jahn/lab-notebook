...
```{r}
df_DCW_20220411 <- read_csv(paste0(dir_exp, "20220411_MC4_Re_mutants_chemostat/20220411_gDCW.csv")) %>%
  mutate(
    DCW_mg = total_weight_mg - tube_weight_mg,
    yield_gDCW_gFRC = DCW_mg/(1000 * 2.0 * 0.05)
  )
...
#  mutate(sample = factor(sample, c("WT", "hoxG", "hoxH", "hoxA", "hypB", "hypX"))) %>%
plot_barchart(df_DCW_20220411, "sample", "yield_gDCW_gFRC", ylim = c(0, 0.6), offset = 0.1, xrot = 35)
```
